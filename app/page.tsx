    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="bg-gradient-to-b from-primary/10 to-background py-12 px-4">
        <div className="max-w-6xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-white px-4 py-2 rounded-full text-sm font-medium text-primary mb-6 shadow-soft">
          ...
          <div className="flex items-center justify-center gap-4">
            <Link 
              href="/games/create"
              className="bg-primary text-white px-6 py-3 rounded-2xl font-semibold hover:bg-primary/90 transition-colors shadow-soft flex items-center gap-2"
            >
              <Gamepad2 className="w-5 h-5" />
              Create a Game
            </Link>
            <Link 
              href="/songs"
              className="bg-white text-foreground px-6 py-3 rounded-2xl font-semibold hover:bg-muted transition-colors shadow-soft flex items-center gap-2"
            >
      ...
      <section className="py-12 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-2xl font-bold text-foreground">Choose a Game Type</h3>
            <Link href="/games" className="text-primary font-medium flex items-center gap-1 hover:underline">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {gameTypes.map((game) => (
              <Link
                key={game.id}
                href={`/games/${game.id}`}
                className="group bg-white rounded-3xl p-6 shadow-soft hover:shadow-lg transition-all hover:-translate-y-1 border-2 border-transparent hover:border-primary/20"
              >
                ...
                <h4 className="text-xl font-bold text-foreground mb-2">{game.name}</h4>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-primary font-medium text-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
      ...
      <section className="py-12 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <h3 className="text-2xl font-bold text-foreground mb-8">Continue Playing</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {recentGames.map((game) => {
              ...
              return (
                <div
                  key={game.id}
                  className="bg-muted/50 rounded-2xl p-5 flex items-center gap-4 hover:bg-muted transition-colors cursor-pointer group"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-foreground truncate">{game.name}</h4>
                    <p className="text-sm text-muted-foreground">{game.lastPlayed}</p>
                  </div>
                  <button className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <section className="py-12 px-4">
        <div className="max-w-6xl mx-auto">
          <div className="bg-gradient-to-r from-primary to-game-coral rounded-3xl p-8 text-white">
